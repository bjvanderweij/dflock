...
> [!CAUTION]
> If you push branches selectively you need to also push downstream branches because otherwise Gitlab will not display diffs correctly. For example, if you update MR 1 and push only that ephemeral branch, Gitlab will no longer recognize the commits in the target branch of MR 2 as being the same as the commits in MR 1 and will show a diff between branch 2 and the upstream (the last common ancestor of ephemeral branches 2 and 3).
